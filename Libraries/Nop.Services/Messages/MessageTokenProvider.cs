...
namespace Nop.Services.Messages
{
    ...
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        #endregion
        public async void AddOrderTokens(IList<Token> tokens, Order order, int languageId, int vendorId = 0)
        {
            tokens.Add(new Token("Order.OrderNumber", order.CustomOrderNumber));
            ...
            tokens.Add(new Token("Order.BillingStateProvince", (order.BillingAddress.StateProvince != null) ? await this._localizationService.GetLocalizedAsync<StateProvince, string>(order.BillingAddress.StateProvince, (StateProvince x) => x.Name, null, true, true) : string.Empty));
            tokens.Add(new Token("Order.BillingZipPostalCode", order.BillingAddress.ZipPostalCode));
            tokens.Add(new Token("Order.BillingCountry", (order.BillingAddress.Country != null) ? await this._localizationService.GetLocalizedAsync<Country, string>(order.BillingAddress.Country, (Country x) => x.Name, null, true, true) : string.Empty));
            tokens.Add(new Token("Order.BillingCustomAttributes", await this._addressAttributeFormatter.FormatAttributesAsync(order.BillingAddress.CustomAttributes, "<br />", true), true));
            tokens.Add(new Token("Order.Shippable", !string.IsNullOrEmpty(order.ShippingMethod)));
            tokens.Add(new Token("Order.ShippingMethod", order.ShippingMethod));
            ...
            string key = "Order.ShippingStateProvince";
            Address shippingAddress = order.ShippingAddress;
            tokens.Add(new Token(key, (((shippingAddress != null) ? shippingAddress.StateProvince : null) != null) ? await this._localizationService.GetLocalizedAsync<StateProvince, string>(order.ShippingAddress.StateProvince, (StateProvince x) => x.Name, null, true, true) : string.Empty));
            ...
            tokens.Add(new Token("Order.ShippingCountry", (order.ShippingAddress != null && order.ShippingAddress.Country != null) ? await this._localizationService.GetLocalizedAsync<Country, string>(order.ShippingAddress.Country, (Country x) => x.Name, null, true, true) : string.Empty));
            tokens.Add(new Token("Order.ShippingCustomAttributes", await this._addressAttributeFormatter.FormatAttributesAsync((order.ShippingAddress != null) ? order.ShippingAddress.CustomAttributes : string.Empty, "<br />", true), true));
            IPaymentMethod paymentMethod =await this._paymentService.LoadPaymentMethodBySystemName(order.PaymentMethodSystemName);
            string value = (paymentMethod != null) ? await this._localizationService.GetLocalizedFriendlyNameAsync<IPaymentMethod>(paymentMethod, (await this._workContext.GetWorkingLanguageAsync()).Id, true) : order.PaymentMethodSystemName;
            tokens.Add(new Token("Order.PaymentMethod", value));
            tokens.Add(new Token("Order.VatNumber", order.VatNumber));
            StringBuilder stringBuilder = new StringBuilder();
            Dictionary<string, object> dictionary = this._paymentService.DeserializeCustomValues(order);
            if (dictionary != null)
            {
                foreach (KeyValuePair<string, object> keyValuePair in dictionary)
                {
                    stringBuilder.AppendFormat("{0}: {1}", WebUtility.HtmlEncode(keyValuePair.Key), WebUtility.HtmlEncode((keyValuePair.Value != null) ? keyValuePair.Value.ToString() : string.Empty));
                    stringBuilder.Append("<br />");
                }
            }
            tokens.Add(new Token("Order.CustomValues", stringBuilder.ToString(), true));
            tokens.Add(new Token("Order.Product(s)", await this.ProductListToHtmlTableAsync(order, languageId, vendorId), true));
            Language languageById = await this._languageService.GetLanguageByIdAsync(languageId);
            if (languageById != null && !string.IsNullOrEmpty(languageById.LanguageCulture))
            {
                tokens.Add(new Token("Order.CreatedOn", this._dateTimeHelper.ConvertToUserTime(order.CreatedOnUtc, TimeZoneInfo.Utc, await this._dateTimeHelper.GetCustomerTimeZoneAsync(order.Customer)).ToString("D", new CultureInfo(languageById.LanguageCulture))));
            }
            else
            {
                tokens.Add(new Token("Order.CreatedOn", order.CreatedOnUtc.ToString("D")));
            }
            string value2 = await this.RouteUrlAsync(order.StoreId, "OrderDetails", new
            {
                orderId = order.Id
            });
            tokens.Add(new Token("Order.OrderURLForCustomer", value2, true));
            await this._eventPublisher.EntityTokensAddedAsync(order, tokens);
        }
    }
}
...
namespace Nop.Services.Payments
{
    ...
    public partial class PaymentService : IPaymentService
    {
        ...
        public async Task<decimal> CalculateAdditionalFee(IList<ShoppingCartItem> cart, decimal fee, bool usePercentage)
        {
            if (fee <= 0M)
            {
                return fee;
            ...
            else
            {
                ...
                {
                    var serviceShoppingCart = await EngineContext.Current.Resolve<IOrderTotalCalculationService>().GetShoppingCartTotalAsync(cart, usePaymentMethodAdditionalFee: false);
                    return (Decimal)(float)((double)(float)serviceShoppingCart.shoppingCartTotal * (double)(float)fee / 100.0);
                }
            }
        }
        async Task<IPaymentMethod> IPaymentService.LoadPaymentMethodBySystemName(string systemName)
        {
            var t = await this._pluginFinder.GetPluginDescriptorBySystemNameAsync<IPaymentMethod>(systemName);
            return t?.Instance<IPaymentMethod>();
        }

        public bool IsPaymentMethodActive(IPaymentMethod paymentMethod)
        {
            if (paymentMethod == null)
                throw new ArgumentNullException(nameof(paymentMethod));
            if (this._paymentSettings.ActivePaymentMethodSystemNames == null)
                return false;
            foreach (string methodSystemName in this._paymentSettings.ActivePaymentMethodSystemNames)
            {
                if (paymentMethod.PluginDescriptor.SystemName.Equals(methodSystemName, StringComparison.InvariantCultureIgnoreCase))
                    return true;
            }
            return false;
        }

        public Task<decimal> CalculateAdditionalFee(IList<ShoppingCartItem> cart, object additionalFee, object additionalFeePercentage)
        {
